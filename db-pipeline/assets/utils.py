...
def keys_check(df, cols_chaves):
    '''
    Função A função keys_check tem a finalidade de realizar análises de integridade dos dados em um DataFrame. Em particular, ela examina 
    a presença de valores ausentes em uma coluna chave tailnum) e valida a singularidade das combinações nas colunas-chave indicadas. 
    INPUT: DataFrame Pandas que contém os dados que serão verificados.
    OUTPUT: Essa função produzirá registros informativos e de alerta.
    '''
